"""Dynamic Variable Partitioning First-Fit Memory Allocator."""
...
class FirstFitAllocator(BaseAllocator):
    """
    Dynamic First-Fit Memory Allocator.
    - Category: Variable partitioning (MVT).
    - Policy: Linear scan through address-ordered free blocks, picking the first block that fits.
    - Splitting: Blocks are split if excess size >= min_split_bytes.
    - Coalescing: Adjacent free blocks are merged upon deallocation.
    - Pathology: Accumulation of small slivers at the beginning of the address space.
    """

    def __init__(self, total_heap_bytes: int = 64 * 1024 * 1024, min_split_bytes: int = 64) -> None:
        super().__init__(name="FirstFit", total_heap_bytes=total_heap_bytes)
        ...
        for idx, block in enumerate(self.blocks):
            if not block.is_allocated and block.size_bytes >= size_bytes:
                ...
                if excess >= self.min_split_bytes:
                    ...
                    self.blocks.insert(idx + 1, new_free_block)
